 ...
 [{:name :text-screen,
  ...
  {:name :text-screen,
   ...
   {:direction 3.1336246947282715,
    :protocol :none,
    :text
    "POGGIO INSTITUTE\n================\n\nIF YOU HAVE TWO BOOLEANS, b1 AND b2, HERE ARE SOME SHORTCUTS TO BUILD NEW BOOLEANS.\n\n(not b1) RETURNS THE OPPOSITE OF b1.\n\n(and b1 b2) RETURNS true ONLY IF BOTH ARE true\n\n(or b1 b2) RETURNS true IF ONE OR BOTH ARE true.",
    :parameter "message",
    :text-color {:red 127, :green 255, :blue 0},
    :z 9,
    :x 15,
  ...
  {:name :text-screen,
   ...
   {:direction 1.5707963267948966,
    :protocol :none,
    :text
    "POGGIO INSTITUTE\n================\n\nHERE ARE A FEW MORE FUNCTIONS THAT WORK WITH BOOLEANS.\n\nany? -- RETURNS true IF ONE ELEMENT IN A LIST PASSES A TEST.\n(any? not [false]) RETURNS true.\n\nall? -- RETURNS true IF ALL ELEMENTS IN A LIST PASSES A TEST.\n(all? id [true false true]) RETURNS false.\n\none? -- RETURNS true IF EXACTLY ONE ELEMENT IN A LIST PASSES A TEST.\n(one? id [true false false]) RETURNS true.",
    :parameter "message",
    :text-color {:red 127, :green 255, :blue 0},
    :z 9,
    :x 26,
